...
import { inviteClientTeamMember } from '../../api/invitations';
...
export default function InviteTeamMemberModal({ onClose, onCreated }: Props) {
  ...
  async function handleSubmit() {
    ...
    setLoading(true);
    try {
      const inv = await inviteClientTeamMember({
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[15px] font-semibold text-gray-900">Inviter un membre</p>
              <p className="text-[12px] text-gray-400">
                {created ? 'Invitation envoyée avec succès.' : 'Il recevra un lien d\'accès par email.'}
              </p>
            </div>
          </div>
          <button
          ...
          {!created ? (
            <div className="space-y-3">
              ...
              <input
                ...
                placeholder="membre@votreentreprise.com"
              ...
              <div className="flex items-start gap-2 px-3 py-2 bg-indigo-50 border border-indigo-100 rounded-lg">
                <p className="text-[12px] text-indigo-700">
                  Ce membre accédera à votre espace : documents, messages et historique.
                </p>
              </div>
            </div>
          ) : (
        ...
        <div className="px-6 pb-6 flex items-center justify-end gap-2">
          {!created ? (
            ...
            <button
              onClick={onClose}
              className="px-4 py-2 text-[13px] font-medium text-white bg-indigo-600
