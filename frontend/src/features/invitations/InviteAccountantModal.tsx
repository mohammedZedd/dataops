...
export default function InviteAccountantModal({ companyName, onClose, onCreated }: Props) {
  ...
  async function handleSubmit() {
    ...
    setLoading(true);
    try {
      const inv = await createAccountantInvitation({
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[12px] text-gray-400">
                {created ? 'Invitation envoyée avec succès.' : 'Le comptable recevra un lien par email.'}
              </p>
            </div>
          </div>
          <button
            ...
            <div className="space-y-3">
              {companyName && (
                <div className="flex items-center gap-2 px-3 py-2 bg-gray-50 border border-gray-100 rounded-lg">
                  <span className="text-[11px] font-medium text-gray-400 uppercase tracking-wider">Cabinet</span>
